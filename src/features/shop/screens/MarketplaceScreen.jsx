    ...
    <View style={styles.container}>
      ...
      <ProductDetailsModal
        ...
        onProceed={(variant, plan) => {
          ...
          const variantName = variant?.name || variant?.label || 'Standard';
          const monthlyAmount = plan?.monthlyInstallment || 0;
          
          Alert.alert(
            'Checkout',
            `Proceeding for ${product?.title || product?.name} (${variantName}) with EMI of ${formatINR(monthlyAmount)}/mo`
          );
        }}
      />
    </View>
